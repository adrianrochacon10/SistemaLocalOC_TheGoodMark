...
export async function solicitarCodigo(entidad, entidadId, user) {
  ...
  if (entidad !== "colaborador" && entidad !== "orden") return { error: "entidad debe ser colaborador u orden" };
  ...
  if (entidad === "colaborador") {
    const { data: colab } = await supabase.from("colaboradores").select("email").eq("id", entidadId).single();
    if (colab?.email?.trim()) destinatarios.add(colab.email.trim());
  }
  if (entidad === "orden") {
    // Usamos "*" para evitar romper si tu BD aún conserva `cliente_id` (fallback).
    const { data: venta } = await supabase.from("ventas").select("*").eq("id", entidadId).single();
  ...
  const tipoEntidad = entidad === "colaborador" ? "un colaborador" : "una venta/orden";
