...
export async function actualizar(id, nombre, precio) {
  ...
  if (!nombreTrim) return { error: "Nombre es obligatorio" };
  const precioNum = Number(precio);
  if (!Number.isFinite(precioNum) || precioNum < 0) {
    return { error: "Precio debe ser un numero mayor o igual a 0" };
  }

  const intentoPrecio = await supabase
    .from("productos")
    .update({ nombre: nombreTrim, precio: precioNum })
    .eq("id", id)
    .select("*")
    .single();
  if (!intentoPrecio.error) return { data: intentoPrecio.data };
  ...
  const errorColumnaPrecio =
    msg.includes("precio") &&
    (msg.includes("does not exist") || msg.includes("schema cache"));
  if (!errorColumnaPrecio) throw new Error(intentoPrecio.error.message);

  const intentoUnitario = await supabase
    .from("productos")
    .update({ nombre: nombreTrim, precio_unitario: precioNum })
    .eq("id", id)
    .select("*")
    .single();
  if (!intentoUnitario.error) return { data: intentoUnitario.data };
  ...
  const intentoMes = await supabase
    .from("productos")
    .update({ nombre: nombreTrim, precio_por_mes: precioNum })
    .eq("id", id)
    .select("*")
    .single();
  if (!intentoMes.error) return { data: intentoMes.data };
